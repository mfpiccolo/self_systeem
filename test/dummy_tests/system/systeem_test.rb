...
SysteemConfig::Features.each do |f|
  ...
  if file_hash[:requirements].last.present?
    db_filename = "./" + SelfSysteem.test_dir + "/dummy_tests" + "/system/support/affirmations/" + file_hash[:requirements].last + "_db.yml"
    session_filename = "./" + SelfSysteem.test_dir + "/dummy_tests" + "/system/support/affirmations/" + file_hash[:requirements].last + "_session.yml"
  end
